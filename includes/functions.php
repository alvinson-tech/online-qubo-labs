...
// UPDATED: Get student's attendance summary for all subjects (replaces VIEW)
function getStudentAttendanceSummary($student_id) {
    ...
    LEFT JOIN attendance_sessions ats ON ats.class_id = st.class_id AND ats.subject_id = sub.subject_id AND ats.status = 'ended'
    LEFT JOIN attendance_records ar ON ar.session_id = ats.session_id AND ar.student_id = st.student_id
...
// UPDATED: Get student's attendance for a specific subject (replaces VIEW)
function getStudentSubjectAttendance($student_id, $subject_id) {
    ...
    LEFT JOIN student_subjects ss ON st.student_id = ss.student_id AND sub.subject_id = ss.subject_id
    LEFT JOIN attendance_sessions ats ON ats.class_id = st.class_id AND ats.subject_id = sub.subject_id AND ats.status = 'ended'
    LEFT JOIN attendance_records ar ON ar.session_id = ats.session_id AND ar.student_id = st.student_id
    WHERE st.student_id = ? AND ss.mapping_id IS NOT NULL
    GROUP BY st.student_id, st.usn_number, st.student_name, st.class_id, sub.subject_id, sub.subject_name, sub.subject_code";
